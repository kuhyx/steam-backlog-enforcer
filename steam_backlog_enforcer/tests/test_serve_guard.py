...
_HOST = "127.0.0.1"
_PORT = 8000
_LOCAL_V4 = f"{_serve_guard._hex_v4(_HOST)}:{_serve_guard._hex_port(_PORT)}"
...
class TestHexRendering:
    ...
    def test_port_is_upper_hex(self) -> None:
        assert _serve_guard._hex_port(8000) == "1F40"

    def test_ipv4_is_little_endian(self) -> None:
        assert _serve_guard._hex_v4("127.0.0.1") == "0100007F"

    def test_ipv6_swaps_each_word(self) -> None:
        assert _serve_guard._hex_v6("::1") == "0" * 24 + "01000000"

    def test_v4_mapped_v6_is_rendered(self) -> None:
        mapped = _serve_guard._hex_v6("::ffff:127.0.0.1")
...
class TestConflictingAddresses:
    ...
    def test_includes_self_wildcard_and_mapped(self) -> None:
        addrs = _serve_guard._conflicting_addresses(_HOST)
        assert _serve_guard._hex_v4(_HOST) in addrs
        assert _serve_guard._WILDCARD_V4 in addrs
        assert _serve_guard._hex_v6("::") in addrs
    ...
    def test_excludes_an_unrelated_interface(self) -> None:
        ...
        assert _serve_guard._hex_v4(
            "192.168.1.5"
        ) not in _serve_guard._conflicting_addresses(_HOST)
...
class TestListeningInodes:
    ...
    def test_finds_matching_listener(self, tmp_path: Path) -> None:
        ...
        with patch_proc(proc):
            assert _serve_guard._listening_inodes(_HOST, _PORT) == {4242}

    def test_finds_wildcard_listener(self, tmp_path: Path) -> None:
        proc = make_proc(tmp_path)
        wildcard = f"{_serve_guard._WILDCARD_V4}:{_serve_guard._hex_port(_PORT)}"
        write_net_tcp(proc, [tcp_row(wildcard, 7)])
        with patch_proc(proc):
            assert _serve_guard._listening_inodes(_HOST, _PORT) == {7}
    ...
    def test_finds_v6_wildcard_listener(self, tmp_path: Path) -> None:
        ...
        proc = make_proc(tmp_path)
        local = f"{_serve_guard._hex_v6('::')}:{_serve_guard._hex_port(_PORT)}"
        write_net_tcp(proc, [tcp_row(local, 9)], v6=True)
        with patch_proc(proc):
            assert _serve_guard._listening_inodes(_HOST, _PORT) == {9}
    ...
    def test_ignores_non_listening_state(self, tmp_path: Path) -> None:
        ...
        with patch_proc(proc):
            assert _serve_guard._listening_inodes(_HOST, _PORT) == set()

    def test_ignores_other_port(self, tmp_path: Path) -> None:
        proc = make_proc(tmp_path)
        other = f"{_serve_guard._hex_v4(_HOST)}:{_serve_guard._hex_port(9999)}"
    ...
    def test_ignores_other_interface(self, tmp_path: Path) -> None:
        proc = make_proc(tmp_path)
        lan = f"{_serve_guard._hex_v4('192.168.1.5')}:{_serve_guard._hex_port(_PORT)}"
...
class TestPidOwnsInode:
    ...
    def test_matching_fd_is_found(self, tmp_path: Path) -> None:
        ...
        assert _serve_guard._pid_owns_inode(pid_dir, {4242}) is True

    def test_unrelated_fds_do_not_match(self, tmp_path: Path) -> None:
        proc = make_proc(tmp_path)
        pid_dir = add_pid(proc, 100, inodes=[1, 2])
        assert _serve_guard._pid_owns_inode(pid_dir, {4242}) is False
    ...
    def test_unreadable_single_fd_is_skipped(self, tmp_path: Path) -> None:
        # A plain file in fd/ cannot be readlink()ed; the scan must continue
        # to the sibling entry rather than abandoning the process.
        proc = make_proc(tmp_path)
...
class TestFindPortOwner:
    ...
    def test_no_listener_returns_none(self, tmp_path: Path) -> None:
        proc = make_proc(tmp_path)
        with patch_proc(proc):
            assert _serve_guard.find_port_owner(_HOST, _PORT) is None
    ...
    def test_resolves_the_listening_pid(self, tmp_path: Path) -> None:
        ...
        with patch_proc(proc):
            assert _serve_guard.find_port_owner(_HOST, _PORT) == 200
...
class TestReadCmdline:
    ...
    def test_returns_argv(self, tmp_path: Path) -> None:
        ...
        with patch_proc(proc):
            assert _serve_guard.read_cmdline(100)[0] == "python"

    def test_missing_process_is_empty(self, tmp_path: Path) -> None:
        proc = make_proc(tmp_path)
        with patch_proc(proc):
            assert _serve_guard.read_cmdline(4321) == []
...
class TestIsOurServer:
    ...
    def test_our_serve_process_matches(self, tmp_path: Path) -> None:
        ...
        with patch_proc(proc):
            assert _serve_guard.is_our_server(100) is True
    ...
    def test_other_subcommand_does_not_match(self, tmp_path: Path) -> None:
        ...
        with patch_proc(proc):
            assert _serve_guard.is_our_server(100) is False
